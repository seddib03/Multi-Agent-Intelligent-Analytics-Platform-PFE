...
def compute_quality_report(
    ...
    # G\u00e9n\u00e9rer le fichier sources.yml (un \u00e9crasement n'est pas g\u00eanant car dbt test est atomique par job 
    # ou on peut isoler par job_id, mais on va assumer execution s\u00e9quentielle pour l'instant ou on isole via env)
    schema_path = str(models_dir / f"_sources_{job_id}.yml")
    ...
    cmd = ["dbt", "test", "--store-failures", "--project-dir", str(dbt_dir), "--profiles-dir", str(dbt_dir), "--select", f"source:staging.raw_data"]
    subprocess.run(cmd, env=env, capture_output=True, text=True) # On ignore le code de retour car dbt remonte les echecs de tests via le JSON.
    
    # 2. R\u00e9cup\u00e9rer nombre total rows via duckdb
    ...
    if run_results_path.exists() and manifest_path.exists():
        ...
        # Grouper les échecs par colonne
        col_failures = {}
        for test in test_results:
            node_id = test["unique_id"]
            node = manifest["nodes"][node_id]
            col_name = node.get("column_name")
            if not col_name: continue
            
            test_type = node.get("test_metadata", {}).get("name")
            failures_count = test.get("failures", 0)

            # Extraire les invalid_rows depuis la table dbt-duckdb
            failed_rows = []
            sample_invalid = []
            if failures_count > 0:
                relation_name = node.get("relation_name")
                if relation_name:
                    # relation_name est du type "database"."schema"."test_alias"
                    try:
                        with duckdb.connect(duckdb_path) as conn:
                            res = conn.execute(f'SELECT __row_id, "{col_name}" FROM {relation_name} LIMIT 2000').fetchall()
                            failed_rows = [int(r[0]) for r in res]
                            sample_invalid = [r[1] for r in res[:5] if r[1] is not None]
                    except Exception as e:
                        logger.error("Impossible de lire les échecs dbt pour la table %s : %s", relation_name, e)
            ...
            if test_type == "not_null":
                ...
                target_key = "type_errors"
                if test_type == "in_range": target_key = "range_errors"
                elif test_type == "accepted_values": target_key = "enum_errors"
                elif test_type == "regex_pattern": target_key = "pattern_errors"
                elif test_type == "date_format": target_key = "date_errors"
                elif test_type == "is_type": target_key = "type_errors"
        ...
        for col_meta in metadata:
            ...
            if not col_meta.nullable:
                ...
                score.completeness_detail = {"null_count": int(null_c), "total": int(total_rows), "null_rows": f["not_null"]["rows"][:500]}
            ...
            has_validity = (col_meta.type in ("int", "float", "date") or col_meta.has_enum or col_meta.has_pattern or col_meta.has_range)
            if has_validity:
                val_c = sum(f[k]["count"] for k in ["type_errors", "range_errors", "enum_errors", "pattern_errors", "date_errors"])
