...
async def test():
    ...
    # ── Étape 2 : Récupérer plan anomalies ─────────────────────
    ...
    # Les anomalies sont dans plan_response["plan"]["anomalies"]
    ...
    for a in anomalies:
        ...
        print(
            ...
            f"action recommandée: {action_detail.get('action', 'flag_only')}"
        )
    ...
    for anomaly in anomalies:
        ...
        # Récupère la clé de l'action recommandée (ex: "action_1")
        recommended_key = anomaly.get("recommended_action", "action_1")

        # Récupère l'action concrète depuis proposed_actions
        ...
        print(f"  → Décision : {anomaly_id} | {chosen_action}")

    if not decisions:
        print("ℹ️ Aucune anomalie — envoi d'une validation vide")
    ...
    print(f"\nHTTP validate : {validate_response.status_code}")
    print(f"status        : {validate_result.get('status')}")

    if validate_response.status_code != 200:
        print(f"❌ Erreur validate : {validate_result}")
        return

    quality_comp = validate_result.get("quality_comparison", {})
    if quality_comp:
        before = quality_comp.get("before", {})
        after  = quality_comp.get("after",  {})
        print(f"quality avant : {before.get('global', 'N/A')}")
        print(f"quality après : {after.get('global',  'N/A')}")
    ...
    if validate_result.get("status") in ["completed", "success"]:
        ...
        print(f"✅ silver : {state.data_prep_paths.get('silver')}")
        print(f"✅ gold   : {state.data_prep_paths.get('gold')}")
    else:
        print(f"⚠️ Statut inattendu : {validate_result.get('status')}")
        print(validate_result)
        return

    # ── Étape 4 : Récupérer data_profile ───────────────────────
