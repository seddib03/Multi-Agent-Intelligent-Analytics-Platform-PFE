  ...
  return (
    <div className="bg-card rounded-xl p-4 border border-border">
      ...
      <ResponsiveContainer width="100%" height={180}>
        {type === "bar" ? (
          <BarChart data={chart.data}><CartesianGrid strokeDasharray="3 3" className="stroke-border" /><XAxis dataKey="name" tick={{ fontSize: 10 }} /><YAxis tick={{ fontSize: 10 }} /><Tooltip />{chart.dataKeys.map((key, i) => (<Bar key={key} dataKey={key} fill={DXC_CHART_COLORS[i % DXC_CHART_COLORS.length]} radius={[4, 4, 0, 0]} />))}</BarChart>
        ) : type === "line" ? (
          <LineChart data={chart.data}><CartesianGrid strokeDasharray="3 3" className="stroke-border" /><XAxis dataKey="name" tick={{ fontSize: 10 }} /><YAxis tick={{ fontSize: 10 }} /><Tooltip /><Legend />{chart.dataKeys.map((key, i) => (<Line key={key} type="monotone" dataKey={key} stroke={DXC_CHART_COLORS[i % DXC_CHART_COLORS.length]} strokeWidth={2} dot={false} />))}</LineChart>
        ) : type === "area" ? (
          <AreaChart data={chart.data}><CartesianGrid strokeDasharray="3 3" className="stroke-border" /><XAxis dataKey="name" tick={{ fontSize: 10 }} /><YAxis tick={{ fontSize: 10 }} /><Tooltip />{chart.dataKeys.map((key, i) => (<Area key={key} type="monotone" dataKey={key} fill={DXC_CHART_COLORS[i % DXC_CHART_COLORS.length]} fillOpacity={0.3} stroke={DXC_CHART_COLORS[i % DXC_CHART_COLORS.length]} />))}</AreaChart>
        ) : (
          <PieChart><Pie data={chart.data} dataKey={chart.dataKeys[0]} nameKey="name" cx="50%" cy="50%" innerRadius={30} outerRadius={60}>{chart.data.map((_, i) => (<Cell key={i} fill={DXC_CHART_COLORS[i % DXC_CHART_COLORS.length]} />))}</Pie><Tooltip /></PieChart>
        )}
      </ResponsiveContainer>
    </div>
  );
        ...
        <tbody>
          {predictions.map((e, i) => (
            <tr key={e.id} className={i % 2 === 0 ? "bg-card" : "bg-muted"}>
              ...
              <td className="px-3 py-2"><div className="flex items-center gap-2"><div className="w-16 h-1.5 bg-muted rounded-full overflow-hidden"><div className="h-full rounded-full" style={{ width: `${e.riskScore}%`, background: e.riskScore > 80 ? "#D14600" : e.riskScore > 60 ? "#FF7E51" : "#004AAC" }} /></div><span className="text-foreground font-semibold">{e.riskScore}%</span></div></td>
              <td className="px-3 py-2 text-foreground">{e.mainFactor}</td>
              <td className="px-3 py-2"><span className={e.trend === "up" ? "text-destructive" : e.trend === "down" ? "text-dxc-gold" : "text-primary"}>{e.trend === "up" ? "↑" : e.trend === "down" ? "↓" : "→"}</span></td>
            </tr>
          ))}
        </tbody>
      </table>
...
function ProcessingIndicator() {
  ...
  useEffect(() => { const interval = setInterval(() => { setActive((prev) => (prev < steps.length - 1 ? prev + 1 : prev)); }, 300); return () => clearInterval(interval); }, []);
  return (
...
export function NLQInterface() {
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (isMobile && e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handleSend(input); }
    else if (!isMobile && (e.ctrlKey || e.metaKey) && e.key === "Enter") handleSend(input);
  };
    ...
    <div className="flex h-screen bg-background">
      ...
      <div className={`${sidebarOpen ? "translate-x-0" : "-translate-x-full"} ${sidebarCollapsed ? "lg:-translate-x-full lg:w-0 lg:overflow-hidden" : "lg:translate-x-0"} fixed lg:relative z-50 lg:z-auto w-[260px] bg-dxc-midnight flex flex-col shrink-0 h-full transition-transform duration-300`}>
        ...
        <div className="px-4 flex-1 overflow-y-auto">
          {/* Dataset info with modify option */}
          <div className="mb-4 bg-dxc-royal/20 rounded-lg p-3 space-y-2">
            ...
            <button onClick={() => { setPhase(1); useAppStore.getState().setOnboardingStep(1); setSidebarOpen(false); }} className="text-xs text-dxc-sky hover:text-dxc-white transition-colors flex items-center gap-1 min-h-[44px]">
          ...
          {messages.length === 0 && (
            <div className="text-center py-8">
              ...
              <div className="max-w-2xl mx-auto grid grid-cols-1 sm:grid-cols-2 gap-2">
                {suggestions.map((q, i) => (
                  <button key={i} onClick={() => handleSend(q)} className="text-left text-sm p-3 rounded-xl border border-border bg-card hover:border-primary/50 hover:bg-primary/5 transition-all flex items-start gap-2 group h-full min-h-[60px]">
            ...
            <div key={msg.id} className={`max-w-3xl ${msg.role === "user" ? "ml-auto" : ""} animate-fade-in`}>
              {msg.role === "user" ? (
                ...
                <div className="bg-card border-l-[3px] border-dxc-melon rounded-r-2xl rounded-bl-2xl shadow-sm p-4 space-y-4">
                  <div className="text-sm text-foreground leading-relaxed whitespace-pre-wrap" dangerouslySetInnerHTML={{ __html: msg.content.replace(/\*\*(.*?)\*\*/g, '<strong class="text-primary">$1</strong>') }} />
                  {msg.charts && <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">{msg.charts.map((chart, i) => (<DXCChart key={i} chart={chart} style={chartStyle} />))}</div>}
                  {msg.predictions && msg.predictions.length > 0 && <PredictionTable predictions={msg.predictions} lang={lang} />}
                  <div className="flex gap-2 pt-2 flex-wrap">
                    <button className="text-xs border border-primary text-primary px-3 py-1.5 rounded-lg hover:bg-primary/5 transition-colors flex items-center gap-1 min-h-[36px]" aria-label={t("exportCsv", lang)}><Download size={12} /> {t("exportCsv", lang)}</button>
                    <button onClick={() => setPhase(3)} className="text-xs bg-primary text-primary-foreground px-3 py-1.5 rounded-lg hover:opacity-90 transition-colors flex items-center gap-1 min-h-[36px]" aria-label={t("dashboard", lang)}><BarChart3 size={12} /> {t("dashboard", lang)}</button>
                    <button onClick={() => togglePin(msg.id)} className="text-xs border border-dxc-gold text-dxc-gold px-3 py-1.5 rounded-lg hover:bg-dxc-gold/10 transition-colors flex items-center gap-1 min-h-[36px]" aria-label={t("pin", lang)}><Pin size={12} /> {t("pin", lang)}</button>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="bg-card border-t border-border p-4">
          <div className="max-w-3xl mx-auto flex gap-3">
            <input
              ...
              className="flex-1 bg-muted rounded-xl px-4 py-3 text-sm text-foreground placeholder:text-foreground/30 border-2 border-transparent focus:border-primary focus:outline-none"
              aria-label={t("askQuestion", lang)}
            />
            <button onClick={() => handleSend(input)} disabled={!input.trim() || processing} className="w-11 h-11 rounded-full bg-dxc-melon text-dxc-white flex items-center justify-center hover:bg-dxc-red transition-colors disabled:opacity-40 shrink-0 self-center min-w-[44px] min-h-[44px]" aria-label="Envoyer">
              <Send size={16} />
            </button>
          </div>
